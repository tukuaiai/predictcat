...
const DEFAULT_CLOB_MARKET_WS = 'wss://ws-subscriptions-clob.polymarket.com/ws/market';
const normalizeMarketWsHost = (raw) => {
    const s = String(raw || '').trim();
    if (!s) return DEFAULT_CLOB_MARKET_WS;

    // 旧 RTDS host：不再提供 clob_market 行情流（本 bot 需要 Market Channel）。
    if (s.includes('ws-live-data.polymarket.com')) return DEFAULT_CLOB_MARKET_WS;

    // 允许用户填 base：wss://ws-subscriptions-clob.polymarket.com/ws/ -> 自动补全 /market
    if (s.endsWith('/ws/') || s.endsWith('/ws')) return `${s.replace(/\/+$/, '')}/market`;
    if (s.endsWith('/ws/market')) return s;
    if (s.endsWith('/ws/market/')) return s.replace(/\/+$/, '');
...
module.exports = {
    ...
    polymarket: {
        // 2025-09+：CLOB Market Channel 已迁移到 ws-subscriptions-clob（旧 ws-live-data 不再支持 clob_market）。
        host: normalizeMarketWsHost(process.env.POLYMARKET_WS_HOST),
        pingInterval: 5000,
        autoReconnect: true,
    ...
    telegram: {
        ...
        rateLimit: {
            ...
            minIntervalMs: Number(process.env.TELEGRAM_RATE_LIMIT_MIN_INTERVAL_MS || 120),
            retryAfterPaddingMs: Number(process.env.TELEGRAM_RATE_LIMIT_RETRY_PADDING_MS || 1000),
            maxRetries: Number(process.env.TELEGRAM_RATE_LIMIT_MAX_RETRIES || 1),
            send: {
                minIntervalMs: Number(
                    process.env.TELEGRAM_RATE_LIMIT_SEND_MIN_INTERVAL_MS
                    || process.env.TELEGRAM_RATE_LIMIT_MIN_INTERVAL_MS
                    || 120
                ),
                retryAfterPaddingMs: Number(
                    process.env.TELEGRAM_RATE_LIMIT_SEND_RETRY_PADDING_MS
                    || process.env.TELEGRAM_RATE_LIMIT_RETRY_PADDING_MS
                    || 1000
                ),
                maxRetries: Number(
                    process.env.TELEGRAM_RATE_LIMIT_SEND_MAX_RETRIES
                    || process.env.TELEGRAM_RATE_LIMIT_MAX_RETRIES
                    || 1
                )
            },
            edit: {
                minIntervalMs: Number(
                    process.env.TELEGRAM_RATE_LIMIT_EDIT_MIN_INTERVAL_MS
                    || process.env.TELEGRAM_RATE_LIMIT_MIN_INTERVAL_MS
                    || 120
                ),
                retryAfterPaddingMs: Number(
                    process.env.TELEGRAM_RATE_LIMIT_EDIT_RETRY_PADDING_MS
                    || process.env.TELEGRAM_RATE_LIMIT_RETRY_PADDING_MS
                    || 1000
                ),
                maxRetries: Number(
                    process.env.TELEGRAM_RATE_LIMIT_EDIT_MAX_RETRIES
                    || process.env.TELEGRAM_RATE_LIMIT_MAX_RETRIES
                    || 1
                )
            }
    ...
    orderbook: {
        enabled: true,                  // 是否启用
        minImbalance: Number(process.env.ORDERBOOK_MIN_IMBALANCE || 3.0),   // 最低失衡比例（默认对齐档位1）
        minDepth: Number(process.env.ORDERBOOK_MIN_DEPTH || 1000),          // 单边最小深度（$1K）
        minLiquidity: Number(process.env.ORDERBOOK_MIN_LIQUIDITY || 20000), // 双边总流动性（默认对齐档位1）
        depthLevels: 3,                // 计算前N档
        cooldown: 60000,                 // 冷却时间（60秒）
        maxSignalsPerHour: Number(process.env.ORDERBOOK_MAX_SIGNALS_PER_HOUR || 9999),
        ...
        dispatch: {
            coalesceByMarket: process.env.ORDERBOOK_DISPATCH_COALESCE !== 'false',
            maxPendingMarkets: Number(process.env.ORDERBOOK_DISPATCH_MAX_PENDING_MARKETS || 200)
        }
    },
