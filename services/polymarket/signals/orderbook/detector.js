...
class OrderbookDetector {
    constructor(config = {}) {
        ...
        this.MIN_LIQUIDITY = config.minLiquidity || 0;       // 最小总流动性
        ...
        if (this.MIN_LIQUIDITY > 0) {
            console.log(`   最小总流动性: $${this.MIN_LIQUIDITY}`);
        }
    ...
    detect(market, analysis) {
        ...
        // 4. 检查总流动性是否足够
        if (this.MIN_LIQUIDITY > 0 && totalLiquidity < this.MIN_LIQUIDITY) {
            return null;
        }
